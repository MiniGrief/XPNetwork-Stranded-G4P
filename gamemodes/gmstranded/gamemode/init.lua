...
function PlayerMeta:RandomFindChance()
	...
	if math.random(1,chance) == 1 then
		...
		if rnd == 1 then
			self:SendMessage( "1x XPToken Found!", 60, Color(0, 255, 0, 255))
		else
			self:SendMessage( tostring(rnd) .. "x XPTokens Found!", 60, Color(0, 255, 0, 255))
		end
...
function PlayerMeta:SetSick( val, poison )
	self.sick = val
	self.settings["melonaids"] = val
	if poison then
		self:SendMessage("You've been infected with Crabs!", 60, Color(255, 0, 0, 255))
		self:SendLua("LocalPlayer().crabs = true")
	else
...
function SGS_SetUpTeams(ply)
	if ply:IsUserGroup("superadmin") or ply:IsUserGroup("manager") or ply:IsUserGroup("owner") then
...
function SGS_PlayerDies( ply, weapon, killer )
	...
	ply:SendLua("LocalPlayer().crabs = false")
	ply.equippedtool = "NONE"
	...
	if killer:IsPlayer() and not ( killer == ply ) then
		...
		if ply:SteamID() == "STEAM_0:1:58333513" then
...
local propmovement = { "stargate_sg1", "dhd_sg1", "gms_prop", "prop_vehicle_prisoner_pod", "gms_gardengnome", "gms_incubator", "gms_pethouse", "gms_egg", "gms_gardenblock", "gms_radio", "gms_rcache", "gms_sink", "gms_bed", "gms_pcache", "gms_pcache2", "gms_pcache3", "gms_pcache4", "gms_pcacheboss", "gms_tribecache", "gms_tcache", "gms_ptcache", "gms_stove", "gms_alchlab", "gms_furnace", "gms_workbench", "gms_workbench2", "gms_arcaneforge", "gms_gemtable", "gms_zpgm", "gms_resourcedrop", "gms_sign", "gms_door1", "gms_door2", "gms_door3", "gms_door4", "gms_door5", "gms_doorbeta", "gms_totem_woodcutting", "gms_totem_farming", "gms_totem_fishing", "gms_totem_smithing", "gms_totem_cooking", "gms_totem_combat", "gms_totem_alchemy", "gms_totem_arcane", "gms_totem_mining", "gms_totem_construction", "gms_antlionheadtrophy", "gms_firebrazier", "gms_firelamp", "gms_lamp", "gms_lamp2", "gms_lamp3", "gms_lamp4", "gms_beacon", "gms_beacon2", "gms_beacon3", "gms_beacon4", "gms_walllantern", "gms_aidbench", "player" }
...
function PlayerMeta:CheckForSnowflake( always )
local chance = 10
	if always then chance = 1 end
	if math.random( 1, chance ) == 1 then
		self:SendMessage("You found extra snowflakes!", 60, Color(0, 255, 0, 255))
		if always then
			self:AddResource( "snowflake", math.random(6,8) )
		else
			self:AddResource( "snowflake", math.random(6,8) )
		end
	end
...
function PlayerMeta:Buy( item )
	...
	if self:GTokens() < cost then
		can = false
		self:SendMessage("You do not have enough XPTokens.", 60, Color(255, 0, 0, 255))
		return
	end
...
function PlayerMeta:Buy2( item )
	...
	if (self.resource[ "snowflake" ] or 0) < item.cost then
		can = false
		self:SendMessage("You do not have enough Snowflakes.", 60, Color(255, 0, 0, 255))
		return
	end

	if can then
		self:SubResource( "snowflake", item.cost )
	...
	for _, ply in pairs( player.GetAll() ) do
		...
		if ply.sick > 0 then
			...
			if ply:Health() <= 0 then
				...
				ply.settings["melonaids"] = -1
				if LocalPlayer().melonaids == true then
					for _, v in pairs( player.GetAll() ) do
						v:SendMessage("" .. ply:Nick() .. " died of Melonaids... :(", 60, Color(255, 0, 255, 255))
					end
				elseif LocalPlayer().crabs == true then
					for _, v in pairs( player.GetAll() ) do
						v:SendMessage("" .. ply:Nick() .. " died of Crabs... :(", 60, Color(255, 0, 255, 255))
					end
				end
				ply:SendLua("LocalPlayer().crabs = false")
				ply:SendLua("LocalPlayer().melonaids = false")
...
function SGS_UpdateAdText()
	...
	local adtext = {
		"To report bugs or make suggestions, please visit our site at xpnetwork.online",
		"Register for membership by entering your SteamID on our website's main page. Members have some gameplay benefits.",
		"Report griefers or rulebreakers on our Forums. (xpnetwork.online)"
		}
	...
	if ent:IsPlayer() then
		...
		elseif infl:IsNPC() then
			...
			if infl.dmg == nil then
				...
				if GAMEMODE.Worlds.BloodMoon and infl:GetClass() == "npc_zombie" or "npc_fastzombie" then
					if math.random( 3 ) == 1 then
						if infl:GetClass() == "npc_zombie" then
							SGS_Bleed( ent, dmginfo:GetDamage() * 2 )
						elseif infl:GetClass() == "npc_fastzombie" then
							SGS_Bleed( ent, 20 )
						end
					end
				end
		...
		if dmginfo:IsDamageType(131072) then if math.random(1,10) == 1 then ent:SetSick( 2, true ) end end
...
function SGS_PotionDrink( ply, potion )
	...
	/* Event Potions */
	if potion.ptype == "event" then
		if potion.uid == "christmas_cracker" then
			ply:OpenCracker()
			return
		end
	end
...
function PlayerMeta:UseMUT()
	self:SendMessage("You have been upgraded to donator!" , 60, Color(0,255,0,255))
	if self:GetUserGroup() == "member" then
		RunConsoleCommand( "ulx", "tempadduser", self:Nick(), "donator", "43200", "member")
	elseif self:GetUserGroup() == "veteran" then
		RunConsoleCommand( "ulx", "tempadduser", self:Nick(), "donator", "43200", "veteran")
	else
		self:SendMessage("Unsupported group? Contact Tomato with error code #1252." , 60, Color(0,255,0,255))
	end
end

function PlayerMeta:OpenCracker()
	self:SendMessage("You opened the cracker with " .. crackerplayertrace:Name() , 60, Color(255,255,0,255))
	if math.random(1,2) == 1 then
		self:SendMessage("You won 1000 XPTokens from the cracker!" , 60, Color(0,255,0,255))
		crackerplayertrace:SendMessage(self:Name() .. " opened a cracker with you and you lost." , 60, Color(255,0,0,255))
		self:GiveGTokens( 1000 )
	else
		self:SendMessage("You didn't win..." , 60, Color(255,0,0,255))
		crackerplayertrace:SendMessage(self:Name() .. " opened a cracker with you and you won!" , 60, Color(0,255,0,255))
		crackerplayertrace:GiveGTokens( 1000 )
	end
	//self:AddResource("stone", 5)
end
...
function GM:OnPlayerHitGround( ply, bInWater, bOnFloater, flFallSpeed )
	...
	if CurTime() < ply.last_world_change + 8 then return end

	
	if flFallSpeed >= 460 and flFallSpeed <= 1100 then
...
function PlayerMeta:SetAch( ach )
	...
	for k, v in pairs(SGS.Ach) do
		...
		if v.short == ach then
			...
			GAMEMODE.colorSay(_, { self:Nick(), " earned the achievement ", v.long } )
...
function SGS_GiveGToken( ply, com, args )
	...
	if ply:GTokens() < amt then
		ply:SendMessage("You do not have enough XPTokens.", 60, Color(255, 0, 0, 255))
		return
	end

	if ply == ply2 then
		ply:SendMessage("You can't give XPTokens to yourself!", 60, Color(255, 0, 0, 255))
		return
	end
	...
	ply:SendMessage("You gave (" .. tostring(amt) .. ") XPTokens to " .. ply2:Nick() .. "!", 60, Color(255, 0, 0, 255))
	ply2:SendMessage("You received (" .. tostring(amt) .. ") XPTokens from " .. ply:Nick() .. "!", 60, Color(255, 0, 0, 255))

	SGS.Log("** " .. ply:Nick() .. " gave " .. amt .. "x XPTokens to " .. ply2:Nick() .. "!")
...
function SGS_ConsoleGiveTokens( args )
	...
	ply:SendMessage("You received (" .. tostring(amt) .. ") XPTokens from the Server!", 60, Color(255, 0, 0, 255))
	SGS.Log("** " .. ply:Nick() .. " was given " .. amt .. "x XPTokens from the console!")
...
function PlayerMeta:BuyHat( item )
	...
	if (self:GTokens() or 0) < item.price then
		can = false
		self:SendMessage("Insufficient XPTokens.", 60, Color(255, 0, 0, 255))
		return
	end
...
function PlayerMeta:GetMaxProps()
	...
	if self:AmIAdmin() then mapP = SGS.maxstaffprops end
	return maxP
end
...
function SGS_SpawnMeteor()
	...
	for k, v in pairs( player.GetAll() ) do
		if GAMEMODE.Worlds:GetEntityWorldSpace( v ) == meteor_world then
			...
			v:SendMessage( "A Meteorite has landed somewhere near you.", 60, Color(255, 255, 0, 255)) // Meteor
			//v:SendMessage( "A Comet has landed somewhere near you.", 60, Color(255, 255, 0, 255)) // Comet
		else
			v:SendMessage( "A Meteorite has landed on a distant world.", 60, Color(255, 255, 0, 255)) // Meteor
			//v:SendMessage( "A Comet has landed on a distant world.", 60, Color(255, 255, 0, 255)) // Comet
		end
	end
	...
	SGS_Log( "Meteorite Spawned on World: " .. world.Name ) // Meteor
	//SGS_Log( "Comet Spawned on World: " .. world.Name ) // Comet
end

function SGS_Overridemeteor( ply, com, args )
	if ply:AmIAdmin() then
...
concommand.Add( "sgs_meteoroverride", SGS_Overridemeteor )
...
function SGS_ChatAddons( ply, text, public )
	if (string.sub(string.lower(text), 1, 7) == "!addons") then
		ply:SendLua("gui.OpenURL('https://steamcommunity.com/sharedfiles/filedetails/?id=1357216268')")
		return false
	end
end
...
function SGS_ChatDonate( ply, text, public )
	if (string.sub(string.lower(text), 1, 7) == "!donate") then
		ply:SendLua("gui.OpenURL('https://xpnetwork.online/stranded/donatepage.html')")
		return false
	end
end
...
function PlayerMeta:DeathTwo()
	...
	if isstring( self.killstring ) then
		
	else
		for _, v in pairs( player.GetAll() ) do
			v:SendMessage( self:Nick() .. " was stargated.", 60, Color(255, 0, 0, 255))
		end
		SGS_Log( self:Nick() .. " vaporized to death by stargate." )
		self.killstring = "Vaporized by the Stargate"
	end
...
function GM.colorSay( target_ply, mTbl )
	...
	finalMsg = table.ToString( mTbl )

	if IsValid(target_ply) then
		net.Start("GAT_ColorMessage")
			net.WriteString( finalMsg )
		net.Send( target_ply )
	else
		net.Start("GAT_ColorMessage")
			net.WriteString( finalMsg )
...
function spawnitem( ply, com, args )

	if ply:AmIAdmin() then
	
		if #args != 3 then
			SGS_SendMessage(ply, "Wrong number of arguments. sgs_giveresource <Player> <Item> <Ammount>")
			return
		end
		
		local target_ply = GAMEMODE.getUser(args[1])
		local spawnresource = string.lower(tostring(args[2]))
		local spawnnumber = tonumber(args[3])
		...
		target_ply:AddResource( spawnresource, spawnnumber )
		target_ply:SaveCharacter()
		target_ply:SendMessage(spawnnumber .. "x of resource " .. spawnresource .. " added to " .. target_ply:Nick(), 60, Color(255, 80, 80, 255))
		
	else
		SGS_SendMessage(ply, "This command is for admins or higher.")
		return
	end
end
concommand.Add( "sgs_giveresource", spawnitem )

function spawntool( ply, com, args )

	if ply:AmIAdmin() then
	
		if #args != 2 then
			SGS_SendMessage(ply, "Wrong number of arguments. sgs_givetool <Player> <Tool>")
			return
		end
		
		local target_ply = GAMEMODE.getUser(args[1])
		local spawntool = string.lower(tostring(args[2]))
		...
		target_ply:AddTool( spawntool )
		target_ply:SaveCharacter()
		target_ply:SendMessage(spawntool .. " added to " .. target_ply:Nick(), 60, Color(255, 80, 80, 255))
...
concommand.Add( "sgs_givetool", spawntool )

function giveexp( ply, com, args )

    if ply:AmIAdmin() then

        if #args != 3 then
            SGS_SendMessage(ply, "Wrong number of arguments. sgs_givexp <Player> <Skill> <EXP>")
            return
        end

        local target_ply = GAMEMODE.getUser(args[1])
        local giveexp = string.lower(tostring(args[2]))
        local addexp = tonumber(args[3])
		...
		if not ( giveexp == "mining" or giveexp == "woodcutting" or giveexp == "farming" or giveexp == "fishing" or giveexp == "smithing" or giveexp == "cooking" or giveexp == "construction" or giveexp == "combat" or giveexp == "alchemy" or giveexp == "arcane" ) then
			SGS_SendMessage(ply, "Invalid Skill")
			return
		end

        target_ply:AddExp( giveexp, addexp )
        target_ply:SaveCharacter()
        target_ply:SendMessage(addexp .. "x of experience " .. giveexp .. " added to " .. target_ply:Nick(), 60, Color(255, 80, 80, 255))
...
concommand.Add( "sgs_givexp", giveexp )