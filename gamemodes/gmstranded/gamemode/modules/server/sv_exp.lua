...
function PlayerMeta:AddExp( skill, amt, shared )
	...
	if not shared then
		...
		if GAMEMODE.Tribes:GetTribeLevel( self ) >= 1 then
			GAMEMODE.Tribes:PlayerExpCapCheck( self )
			...
			if GAMEMODE.Tribes.ExpCaps[ self:GetPlayerID() ] < expcapnum then
...
function PlayerMeta:AnnounceCap( skill ) --Individual Skill Cap
	...
	GAMEMODE.colorSay(_, { self:Nick(), " has achieved max level of ", skill, "!!" } )
...
function PlayerMeta:AnnounceMilestone( amt ) --Every 10 Survival Levels
	...
	GAMEMODE.colorSay(_, { self:Nick(), " has achieved survival level ", tostring(amt), "!!" } )
...
function SGS_ConSetBonusExp( ply, _, args )
	...
	for k, v in pairs( SGS.skills ) do
		if v == string.lower(args[1]) then
			...
			GAMEMODE.colorSay(nil, { "Bonus EXP for skill ", "(", Cap(v), ")", " is now set to ", args[2], "%", "." } )
			return
		end
	end
end
...
function SGS_ConUnSetBonusExp( ply, _, args )
	...
	for k, v in pairs( SGS.skills ) do
		if v == string.lower(args[1]) then
			...
			GAMEMODE.colorSay(nil, { "Bonus EXP for skill ", "(", Cap(v), ")", " has reset." } )
			return
		end
	end
end
	...
	for k, v in pairs( SGS.skills ) do
		if SGS.bonusexp[v] then
			if SGS.bonusexp[v].enabled then
				GAMEMODE.colorSay(ply, { "Bonus EXP for skill ", "(", Cap(v), ")", " is currently set to ", tostring(SGS.bonusexp[v].mod), "%", "." } )
			end
		end
	end
end
...
function SGS_RandomSkill( ply, _, args )
	...
	GAMEMODE.colorSay(nil, { "Bonus EXP for skill ", "(", Cap(skill), ")", " is now set to ", tostring(percent), "%", "." } )
...
function SGS_SetAllSkillsBonus( ply, _, args )
	...
	for k, v in pairs( SGS.skills ) do
		...
		GAMEMODE.colorSay(nil, { "Bonus EXP for skill ", "(", Cap(v), ")", " is now set to ", tostring(percent), "%", "." } )
	end
...
//concommand.Add( "tribe_checkexpcap", GM.Tribes:PlayerExpCapCheck( self ) )