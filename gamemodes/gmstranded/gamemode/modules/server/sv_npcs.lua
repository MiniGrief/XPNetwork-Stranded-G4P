...
function SGS_MakeCreature( npcclass, loc, aggrotable, parent )
	if SGS_ReverseNPCLookup(npcclass) then
		...
		if npcclass == "npc_zombie" or npcclass == "npc_fastzombie" or npcclass == "npc_headcrab" or npcclass == "npc_headcrab_fast" or npcclass == "npc_poisonzombie" then
...
function SGS_BossHurt( boss, amt )
	...
	if SGS_ReverseNPCLookup(npc:GetClass()) then
		...
		for k, v in pairs( ptable ) do
			...
			if npc:GetClass() == "npc_zombie" then
				...
				local headcrabchance = 5
				if math.random(chance) == 1 then
					SGS_MakeCreature( "npc_headcrab", npc:GetPos() )
				end
			end
			
			if npc:GetClass() == "npc_zombie_fast" then
				local chance = 10
				if npc.bloodmoon_spawned then chance = 7 end
				if math.random(chance) == 1 then
					v:SendMessage( "You managed to snag a usable strip of cloth.", 60, Color(0, 255, 0, 255))
				...
				local headcrabchance = 5
				if math.random(chance) == 1 then
					SGS_MakeCreature( "npc_headcrab_fast", npc:GetPos() )
				end
			end
			...
			if npc:GetClass() == "npc_headcrab" then
				local chance = 2
				if math.random(chance) == 1 then
					v:SendMessage( "You managed to tear off some meat from the headcrab.", 60, Color(0, 255, 0, 255))
					v:AddResource( "headcrab_meat", 1 )
				end
			end
			
			if npc:GetClass() == "npc_headcrab_fast" then
				local chance = 2
				if math.random(chance) == 1 then
					v:SendMessage( "You managed to tear off some meat from the headcrab.", 60, Color(0, 255, 0, 255))
					v:AddResource( "headcrab_meat", 1 )
		...
		if npc:GetClass() == "npc_poisonzombie" then
			...
			local ED = EffectData()
			ED:SetOrigin( pos )
			local effect = util.Effect( 'blood_boss_die', ED, true, true )
		end
	end	
end
...
function SGS_NPCLoot( ltype, npc, ply )
	...
	elseif ltype == "pzombie" then
		...
		/* Boss Pickaxe Head 1/30 chance */
		if math.random(1,30) == 1 then
			ent:SetResourceDropInfo( "congealed_blood", 1 )
			raredrop = true
		end			
		/* Boss Weapon Core 1/30 chance */
		if math.random(1,30) == 1 then
			ent:SetResourceDropInfo( "enchanted_wood", 4 )
			raredrop = true 
		end	
		/* Chaos Stones 1/10 chance */
		if math.random(1,10) == 1 then ent:SetResourceDropInfo( "chaos_stone", 20 ) end		
		/* Nature Stones 1/15 chance */
		if math.random(1,15) == 1 then ent:SetResourceDropInfo( "nature_stone", 20 ) end		
		/* Cosmic Stones 1/20 chance */
		if math.random(1,20) == 1 then ent:SetResourceDropInfo( "cosmic_stone", 20 ) end		
		/* Sapphire 1/8 chance */
		if math.random(1,8) == 1 then ent:SetResourceDropInfo( "sapphire", math.random(4,16 ) ) end		
		/* Emerald 1/8 chance */
		if math.random(1,8) == 1 then ent:SetResourceDropInfo( "emerald", math.random(4,16 ) ) end		
		/* Ruby 1/12 chance */
		if math.random(1,12) == 1 then ent:SetResourceDropInfo( "ruby", math.random(3,12 ) ) end		
		/* Diamond 1/18 chance */
		if math.random(1,18) == 1 then ent:SetResourceDropInfo( "diamond", math.random(2,6) ) end
		
		if raredrop == false then
			local chance = math.random(1, 100)
			
			if chance == 1 then
				ent:SetResourceDropInfo( "congealed_blood", 2 )
			elseif chance > 1 and chance <= 4 then
				ent:SetResourceDropInfo( "congealed_blood", 3 )
			elseif chance > 4 and chance <= 7 then
				ent:SetResourceDropInfo( "congealed_blood", 4 )
			elseif chance > 7 and chance <= 10 then
				ent:SetResourceDropInfo( "congealed_blood", 6 )
			elseif chance > 10 and chance <= 15 then
				ent:SetResourceDropInfo( "enchanted_wood", 5 )
			end
		end
...
function SGS_SpawnZombieBloodMoon()
	...
	if #ents.FindByClass("npc_fastzombie") > 50 then return end

	for k, v in pairs( player.GetAll() ) do
		
		if GAMEMODE.Worlds:CountEntitiesOnWorld( GAMEMODE.Worlds:GetEntityWorldSpaceID( v ), "npc_zombie" ) >= 20 then continue end
		if GAMEMODE.Worlds:CountEntitiesOnWorld( GAMEMODE.Worlds:GetEntityWorldSpaceID( v ), "npc_fastzombie" ) >= 20 then continue end
		if not GAMEMODE.Worlds:GetWorld( v ) then continue end
		if not GAMEMODE.Worlds:GetWorld( v ).ZombieSpawns then continue end
		if not IsValid(v) then continue end
		if not v:Alive() then continue end
		...
		if not (tr.MatType == MAT_SLOSH) then
			if tr.MatType == MAT_DIRT or tr.MatType == MAT_SAND or tr.MatType == MAT_GRASS or tr.MatType == MAT_SNOW then
				if tr.HitWorld then
					...
					if GAMEMODE.Worlds:GetEntityWorldSpace( ply ).pvp then
						SGS_MakeCreature( "npc_fastzombie", npos )
					else
...
function SGS_HeadcrabManagement()
	for k, v in pairs( ents.FindByClass("npc_head*" ) ) do
		if v.ispet != nil then if v.ispet then continue end end
		if v:GetModelScale() <= 1.01 then
			timer.Simple( 0.9, function()
				SGS_MakeCreature( "npc_headcrab_black", v:GetPos(), player.GetAll(), ents.FindByClass( "npc_poisonzombie" )[1] )
				SafeRemoveEntity( v )
			end )
		end
		if GAMEMODE.Worlds:GetEntityWorldSpace( v ).Name == "Arena" then continue end
		
		//SafeRemoveEntity( v )
	end
end
timer.Create( "HeadcrabManagement", 1, 0, SGS_HeadcrabManagement )

function SGS_KillAllPoisionZombies()
	for k, v in pairs( ents.FindByClass( "npc_poisonzombie" ) ) do
		v:Remove()
	end
end

function SGS_BossHealth()
...
function SGS_PrepPoisonZombie_Con()
	
	DayLight.Minute = 1199
	DayLight:ChangeTime()
	net.Start("sgs_settimeupdate")
		net.WriteInt( DayLight.Minute, 16 )
	net.Broadcast()
	SGS.bossspawnedtoday = false
	GAMEMODE.Worlds.lastbloodmoon = 0
end
concommand.Add( "sgs_preppoisonzombie", SGS_PrepPoisonZombie_Con )