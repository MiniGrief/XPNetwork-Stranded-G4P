...
function SPropProtection.PlayerCanTouch(ply, ent)
	...
	if(ent:GetNetworkedString("Owner") == "World") then
		if(ply:AmIAdmin() and tonumber(SPropProtection.Config["awp"]) == 1 and tonumber(SPropProtection.Config["admin"]) == 1) then
			return true
		end
	elseif(ply:AmIAdmin() and tonumber(SPropProtection.Config["admin"]) == 1) then
		return true
	end
...
function SPropProtection.Disconnect(ply)
	...
	if(tonumber(SPropProtection.Config["dpd"]) == 1) then
		if ply:IsUserGroup("moderator") then
			timer.Create("SPropProtection.DRemove: "..sid, 86400, 1, function() SPropProtection.DRemove( sid, sname ) end )
		elseif ply:IsUserGroup("admin") or ply:IsUserGroup("manager") or ply:IsUserGroup("owner") or ply:IsUserGroup("superadmin") then
			return
		end
		if ply:IsMember() then
			if ply:IsDonator() then
				timer.Create("SPropProtection.DRemove: "..sid, 57600, 1, function() SPropProtection.DRemove( sid, sname ) end )
			else
...
function SPropProtection.PhysGravGunPickup(ply, ent)
	...
	if SPropProtection.KVcantouch[ent:EntIndex()] == 2 or (SPropProtection.KVcantouch[ent:EntIndex()] == 1 and ply:AmIAdmin()) then
		return
	end
	if(ent:IsPlayer() and ply:AmIAdmin() and tonumber(SPropProtection.Config["admin"]) == 1) then
		return
	end
...
function SPropProtection.CanTool(ply, tr, mode)
	...
	if not SPropProtection.KVcanuse[ent:EntIndex()] then SPropProtection.KVcanuse[ent:EntIndex()] = -1 end
	if(!SPropProtection.PlayerCanTouch(ply, ent) or SPropProtection.KVcantool[ent:EntIndex()] == 0 or (SPropProtection.KVcantool[ent:EntIndex()] == 1 and !ply:AmIAdmin())) then
		return false
	elseif(mode == "nail") then
		...
		if(tr2.Hit and IsValid(tr2.Entity) and !tr2.Entity:IsPlayer()) then
			if(!SPropProtection.PlayerCanTouch(ply, tr2.Entity) or SPropProtection.KVcantool[tr2.Entity:EntIndex()] == 0 or (SPropProtection.KVcantool[tr2.Entity:EntIndex()] == 1 and !ply:AmIAdmin())) then
				return false
			end
		end
...
function SPropProtection.PlayerUse(ply, ent)
	...
	if not SPropProtection.KVcanuse[ent:EntIndex()] then SPropProtection.KVcanuse[ent:EntIndex()] = -1 end
	if SPropProtection.KVcanuse[ent:EntIndex()] == 0  or (SPropProtection.KVcantouch[ent:EntIndex()] == 1 and !ply:AmIAdmin()) then
		return false
	end
...
function SPropProtection.CDP(ply, cmd, args)
	if( IsValid(ply) and !ply:AmIAdmin() ) then
...
function SPropProtection.CleanupProps(ply, cmd, args)
	...
	elseif( !IsValid(ply) or ply:AmIAdmin()) then
		for k,v in pairs(player.GetAll()) do
