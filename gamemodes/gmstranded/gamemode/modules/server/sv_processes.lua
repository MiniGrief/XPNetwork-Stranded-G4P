...
function SGS_Lumber_Stop(ply, tree, lvl, modi, gem, void)
	...
	local npos = ( (ply:GetPos() + (ply:GetAngles():Forward() * 20)) + (ply:GetAngles():Right() * math.random(-20,0)) ) + Vector(20,20,70)
	local xp = 25
	local modifier = 1
...
function SGS_MeteorMine_Stop(ply, res, modi, gem, void)
	...
	if not IsValid( res ) then
		ply:SendMessage("The meteor is gone", 60, Color(255, 0, 0, 255)) // Meteor
		//ply:SendMessage("The comet is gone", 60, Color(255, 0, 0, 255)) // Comet
		return
	end
	...
	if ( math.random(1,100) * modi ) * lvlmodi >= 80 then		
		...
		/*elseif math.random(chance) == 1 then
			ply:AddResource( "christmas_cracker", 1 ) // Comet
			SGS.Log("** " .. ply:Nick() .. " found a christmas cracker!")*/ // Comet
		else
			local lootaquired = loot_table[math.random(#loot_table)]
			if lootaquired == "snowflake" then
				ply:AddResource( "snowflake", math.random(1,5) )
			else
				ply:AddResource( lootaquired, math.random(5,10) )
			end
		end

		ply:CheckForGem(20)
		ply:CheckForStone( true )
		//ply:CheckForSnowflake( false ) // Comet
...
function SGS_DrinkPotion_Start(ply, len, potion)
	...
	if potion.ptype == "event" then
		if potion.uid == "christmas_cracker" then
			crackerplayertrace = ply:GetEyeTrace().Entity
			if !crackerplayertrace:IsPlayer() or (ply:GetPos():Distance(crackerplayertrace:GetPos()) > 100) then
				ply:SendMessage("You must look at someone to open the cracker with them.", 60, Color(255,0,0,255))
				return
			end
		end
	end
