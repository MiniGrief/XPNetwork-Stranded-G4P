...
hook.Add( "DayLightChangeTime", "DayChangeBossCheck", function( time )
	...
	if minute < 1200 then return end
...
function SGS_SpawnPoisonZombieBoss()
	SGS_MakeCreature( "npc_poisonzombie", GAMEMODE.Worlds.tblWorlds[8].BossSpawnPos + Vector(200,0,0), nil )
	for k, v in pairs(player.GetAll()) do
		v:SendMessage("The Blood Poison Zombie has risen in the Arena!", 60, Color(255, 255, 0, 255))
	end
end
...
function SGS_TeleportBossArena( npc )
	...
	local newpos = Vector(math.random(-9603, -12940), math.random(3961, 14), 7847)
...
function SGS_NPCBossDamage( ent, dmginfo )
	...
	elseif ent:GetClass() == "npc_poisonzombie" then
		if math.random( 1, 10 ) == 1 then
			ent:EmitSound("npc/zombie_poison/pz_call1.wav", 250, 50)
			SGS_SpawnHeadcrabs( ent )
		end
		if math.random( 1, 10 ) == 1 then
			ent:EmitSound("npc/zombie_poison/pz_call1.wav", 250, 50)
			SGS_SpawnZombies( ent )
		end
	end
end
...
function SGS_BossDamageHandling( ent, dmginfo )
	...
	if ent:IsPlayer() then
 	...
 	/*if ent:GetClass() == "npc_headcrab" or ent:GetClass() == "npc_headcrab_fast" or ent:GetClass() == "npc_headcrab_black" or ent:GetClass() == "npc_zombie" or ent:GetClass() == "npc_fastzombie" then
		if IsValid(ent.parent) then
			SGS_BossHurt( ent.parent, amount / 2 )
		end
	end*/
end
...
function BossKillPlayer( ply, wep, killer )
	...
	if killer:GetClass() == "npc_headcrab" then
		SGS_MakeCreature( "npc_zombie", killer:GetPos(), nil, killer.parent )
		SafeRemoveEntity( killer )
		if IsValid(killer.parent) then
			killer.parent:EmitSound("npc/zombie_poison/pz_pain" .. math.random(1,3) .. ".wav", 125)
			SGS_BossHeal( killer.parent, killer.parent:GetMaxHealth() / 10 )
		end
	end
	
	if killer:GetClass() == "npc_headcrab_fast" then
		SGS_MakeCreature( "npc_fastzombie", killer:GetPos(), nil, killer.parent )
		SafeRemoveEntity( killer )
		if IsValid(killer.parent) then
			killer.parent:EmitSound("npc/zombie_poison/pz_pain" .. math.random(1,3) .. ".wav", 125)
			SGS_BossHeal( killer.parent, killer.parent:GetMaxHealth() / 10 )
	...
	if killer:GetClass() == "npc_headcrab_black" then
		SGS_MakeCreature( "npc_fastzombie", killer:GetPos(), nil, killer.parent )
		SafeRemoveEntity( killer )
		if IsValid(killer.parent) then
			killer.parent:EmitSound("npc/zombie_poison/pz_pain" .. math.random(1,3) .. ".wav", 125)
...
function HunterPhase2( npc )
	...
	npc:SetPos( Vector(-11210, 1991, 7775) )
...
function SGS_SpawnHeadcrabs( npc )
	for i = 1, math.random(6,8) do
		SGS_SpawnHeadcrab( npc )
	end
end

function SGS_SpawnHeadcrab( npc )
	if not IsValid( npc ) then return end
	cpos = npc:GetPos()
	npos = cpos + Vector(math.random(-300,300), math.random(-300,300), 500)
	...
	if tr.Hit then
		if not (tr.MatType == MAT_SLOSH) then
			if tr.HitWorld then
				npos = tr.HitPos + Vector(0,0,10)
				if math.random(3) == 1 then
					SGS_MakeCreature( "npc_headcrab_fast", npos, npc.ptable, player.GetAll(), ents.FindByClass( "npc_poisonzombie" )[1] )
				elseif math.random(2) == 1 then
					SGS_MakeCreature( "npc_headcrab_black", npos, npc.ptable, player.GetAll(), ents.FindByClass( "npc_poisonzombie" )[1] )
				else
					SGS_MakeCreature( "npc_headcrab", npos, npc.ptable, player.GetAll(), ents.FindByClass( "npc_poisonzombie" )[1] )
				end
			end
		end
	end
...
function SGS_SpawnZombies( npc )
	for i = 1, math.random(6,8) do
		SGS_SpawnZombie( npc )
	end
end

function SGS_SpawnZombie( npc )
	if not IsValid( npc ) then return end
	cpos = npc:GetPos()
	npos = cpos + Vector(math.random(-300,300), math.random(-300,300), 500)
	...
	if tr.Hit then
		if not (tr.MatType == MAT_SLOSH) then
			if tr.HitWorld then
				npos = tr.HitPos + Vector(0,0,10)
				if math.random(3) == 1 then
					SGS_MakeCreature( "npc_fastzombie", npos, npc.ptable, player.GetAll(), ents.FindByClass( "npc_poisonzombie" )[1] )
				else
					SGS_MakeCreature( "npc_zombie", npos, npc.ptable, player.GetAll(), ents.FindByClass( "npc_poisonzombie" )[1] )
				end
			end
		end
	end
