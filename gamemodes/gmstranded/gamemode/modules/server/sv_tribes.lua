...
util.AddNetworkString("tribes_sv_sendcap")
--Client to Server
...
util.AddNetworkString("tribes_cl_requestcap")

net.Receive("tribes_cl_requestcap", function( len, ply )
	if GAMEMODE.Tribes.ExpCaps[ ply:GetPlayerID() ] == nil then return end
	net.Start("tribes_sv_sendcap")
		net.WriteInt(GAMEMODE.Tribes.ExpCaps[ ply:GetPlayerID() ], 32)
	net.Send(ply)
end )
...
net.Receive( "tribes_cl_admindisband", function( l, ply )
	if not ply:AmIAdmin() then return end
...
function GM.Tribes:AddExp( t_id, amt )
	...
	//SetGlobalInt("curexpcap",GAMEMODE.Tribes.ExpCaps[ self:GetPlayerID() ])
end
...
function CheckExperienceResetTime()
	if os.date("%H") + 0 == 0 and os.date("%M") + 0 == 0 then
