...
function PlayerMeta:GiveGTokens( amt )
	...
	self:SetGTokens(ngt)
	self:SendMessage("You received " .. amt .. "x XPTokens.", 60, Color(0, 255, 0, 255))
	...
	if !IsValid( ply ) or ply:IsSuperAdmin() then
		...
		for k, v in pairs(player.GetAll()) do
			if v:GetPlayerID() == SID then
				v:GiveGTokens(toadd)
				v:SaveCharacter()
				SGS_SendMessage(ply, "User found and XPTokens added")
				SGS.Log("** (CONSOLE) gave " .. toadd .. "x XPTokens to " .. v:Nick() .. "!")
				return
			end
		end
		...
		if file.Exists("sgstranded/newsaves/"..SID..".txt", "DATA") then
			...
			SGS_SendMessage(ply, "User found and XPTokens added")
			SGS.Log("** (CONSOLE) gave " .. toadd .. "x XPTokens to " .. SID .. "!")
		else
