...
function SGS_CheckOnShop2()
	...
	if(ents.FindByClass("npc_breen") ~= false) then
		return
	end
...
function SGS_CheckOnShop3()
	...
	if(ents.FindByClass("npc_kleiner") ~= false) then
		return
	end
...
function SGS_MoveShop(ent)
	...
	if ent:GetPos():DistToSqr( SGS.shopentstart ) > 400000 then
	...
	//local newpos = Vector(9637, 8801, 9656)
...
timer.Create("shop_npc_talkrandomly", 150, 0, SGS_VortTalkChat)
...
function SGS_VortChat( message )
	...
	GAMEMODE.colorSay(_, { "**VORTY**", message } )

end

function SGS_BreenChat( message )

	GAMEMODE.colorSay(_, { "**BREEN**", message } )

end

function SGS_KlienerChat( message )

	GAMEMODE.colorSay(_, { "**DRK**", message } )

end

function SGS_SniperChat( message )

	GAMEMODE.colorSay(_, { "**GUARD**", message } )
...
function SGS_VortyMemberChatWithDrK()

	SGS_VortChat( "Hey Dr. K. Do you know how players become members? I forgot." )
	timer.Simple( 3, function() SGS_KlienerChat( "They press F6 and click Become a Member on the left." ) end )
	timer.Simple( 5, function() SGS_KlienerChat( "Then they paste in their SteamID and click Submit to recieve the rank!" ) end )
	timer.Simple( 8, function() SGS_VortChat( "Thanks for reminding me." ) end )
...
function SGS_ChatVorty( ply, text, public )
    if ( string.sub( string.lower(text), 1, 6 ) == "!vorty" ) then
		...
		if ply:GetUserGroup() == "moderator" then
		 ...
		 GAMEMODE.colorSay(_, { text:sub(8) } )
