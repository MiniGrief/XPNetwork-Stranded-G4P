...
function GM.Tribes:PlayerExpCapCheck( ply )
	local t_id = self:GetPlayersTribe( ply )
	if not self:IsValid( t_id ) then expcapnum = 0 end
	
	expcapnum = GAMEMODE.Tribes.CapList[tonumber(self.tblTribes[ t_id ].level)]
	//curexpcap = GAMEMODE.Tribes.ExpCaps[ ply:SteamID64() ]
end
