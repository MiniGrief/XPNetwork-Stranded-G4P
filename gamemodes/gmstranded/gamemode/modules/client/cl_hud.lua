net.Receive( "sv_restartwarning", function( pl )
	if ( IsValid( pl ) and pl:IsPlayer() ) then
		print("Networking recieved from client with nickname " .. pl:Nick() .. "???")
		print("Report this to Tomato!!!")
	else
		restarting = true
		RestartTimer = net.ReadString()
	end
end )
...
function SGS_RestartWarning()
	if restarting == true then
		...
		surface.SetTextPos( x - 55 , y - 65 )
		surface.SetTextColor( 255, 0, 0, 255 )
		surface.DrawText( "RESTART SOON!" )
		surface.SetFont( "SGS_NEWHUD2" )
		surface.SetTextPos( x - 50 , y - 42 )
		surface.SetTextColor( 255, 255, 255, 255 )
		surface.DrawText( "Time: " .. RestartTimer )
		surface.SetFont( "SGS_NEWHUD1" )
		surface.SetTextPos( x - 45 , y - 30 )
		surface.DrawText( "Pack up your stuff!" )
	end
end
hook.Add( "HUDPaint", "SGS_RestartWarning", SGS_RestartWarning )

net.Receive( "sv_resetrestartwarning", function( pl )
	restarting = false
end )

function HideHUD(name)
	for k, v in pairs( { "CHudHealth", "CHudBattery", "CHudAmmo", "CHudSecondaryAmmo", "CHudWeaponSelection", "CHudHistoryResource", "CHudCrosshair", "CHudPoisonDamageIndicator" } ) do
...
function PANEL:PaintOver()
	...
	if self.btype == "spell" then
		for _, v in pairs( SGS.SpellList ) do
			if v.spell == SGS.HotBarcontents[self.index] then
				local count = LocalPlayer():HowManyCasts(v) or 0
				local len = string.len(tostring(count))
				draw.SimpleText(tostring(count), "HotBarnumbers", 38 - ( (len*7) - 7 ), 34, Color(255, 255, 0, 255), 0, 0)
			end
		end
...
function SGS_DeathHUD()
	...
	draw.DrawText( "Press Jump (Default: Space) to " .. keyword .. "!", "DeathNotice4", ScrW() / 2, ScrH() / 2 - 210, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
...
CreateClientConVar( "sgs_disablehudnumbers", "0", true, false, "Disable the HUD numbers for health, food, thirst and fatigue." )
...
crabsIcon = Material( "vgui/hud/Crabs.png" )
function SGS_AllThingsHUD()
	...
	if LocalPlayer().crabs then
		surface.SetDrawColor( 255, 255, 255, 200 )
		surface.SetMaterial( crabsIcon	)
		surface.DrawTexturedRect( 130 , y - 175, 24, 24 )
	end
	...
	if GetConVar("sgs_disablehudnumbers"):GetBool() != true then
		surface.SetFont( "SGS_NEWHUD2" )
		local message = tostring(math.ceil(( LocalPlayer():Health()))) .. "/" .. tostring(math.ceil(LocalPlayer():GetMaxHealth()))
	...
	if SGS.commandsmenuopen == true then	
		...
		surface.DrawText( "!givetoken - Gives XPTokens to other players." )
	end	
