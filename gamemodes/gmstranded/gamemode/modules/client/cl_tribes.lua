...
net.Receive( "tribes_sv_networkpart", function( l )
	...
	if to_edit == 0 then
		...
		timer.Simple( 1, function() GAMEMODE.Tribes.tblTribes[ t_id ].exp = new_exp end )
	elseif to_edit == 1 then
		...
		timer.Simple( 1, function() GAMEMODE.Tribes.tblTribes[ t_id ].expcontributions[ ply:SteamID() ] = new_exp end )
	end
...
function PANEL:DrawFrame()
	...
	if ( GAMEMODE.Tribes:PlayerInTribe( LocalPlayer() ) ) and LocalPlayer():GetTribe().level > 0 then
		...
		tr_panel.Paint = function()
			GAMEMODE.Tribes:PlayerExpCapCheck( LocalPlayer() )
			...
			net.Start("tribes_cl_requestcap")
				net.WriteString("Hi")
			net.SendToServer()
			net.Receive("tribes_sv_sendcap", function()
				myexpcap = net.ReadInt(32)
			end )
			
			if myexpcap == nil then
				myexpcap = 10
			end
			...
			draw.DrawText("Daily Cap:", "TribeMenu1", 30, 118, Color(255,255,255,255), TEXT_ALIGN_LEFT)
			draw.DrawText(expcapnum, "TribeMenu1", 120, 118, Color(255,255,255,255), TEXT_ALIGN_LEFT)	
			draw.DrawText("Your Cap:", "TribeMenu1", 30, 128, Color(255,255,255,255), TEXT_ALIGN_LEFT)
			draw.DrawText(myexpcap, "TribeMenu1", 120, 128, Color(255,255,255,255), TEXT_ALIGN_LEFT)	
