...
function SPELLS_TAB:DrawFrame()
	...
	local function DrawSpellsNew()
		...
		for _, v in pairs( SGS.SpellList ) do
			...
			if (LocalPlayer():HowManyCasts(v) == 0) and (SGS.levels["arcane"] >= v.reqlvl) then
				draw.RoundedBoxEx( 0, 35, 18, 103, 12, Color(255, 255, 0, 100), false, false, false, false )
				draw.SimpleText("NO STONES", "qMenuFont2", 60, 18, Color(255, 255, 255, 255), 0, 0)
				SpellButton.OnCursorEntered = function()
					return true
				end
			elseif SGS.levels["arcane"] >= v.reqlvl then
			...
			if SGS.levels["arcane"] < v.reqlvl then
				draw.RoundedBoxEx( 0, 0, 0, 140, 34, Color(0, 0, 0, 120), false, false, false, false )
				draw.RoundedBoxEx( 0, 35, 18, 103, 12, Color(255, 0, 0, 100), false, false, false, false )
				draw.SimpleText("INSUFFICIENT SKILL", "qMenuFont2", 37, 17, Color(0, 0, 0, 255), 0, 0)
			end
			end
		...
		if(table.Count(Players) == 1) then
			...
			for k, ply in pairs(Players) do
				if(ply and ply:IsValid() and ply != LocalPlayer()) then
					...
					if GetConVar(FriendCommand) == 1 then
						checkbox:Toggle()
					end
