		...
		if v:GetNWBool("ispet") == true then
			...
			if trace.HitWorld then
				...
				if tdist <= 600 then
					...
					local pos = v:GetPos() + Vector(0,0,v:OBBMaxs().z + 5 + zadj)
					pos = pos:ToScreen()
					local petname = v:GetNWString("petname")
					
					if string.len(petname) > 16 then
						petname = string.SetChar( petname, 17, "#" )
						petname = string.Replace( petname, "#", "..." )
						petname = string.sub( petname, 1, 19 )
					end
					
					if v != LocalPlayer() then
						draw.SimpleTextOutlined( petname, "SGS_HUD3", pos.x, pos.y - 35, Color(80,80,190,255), textalign, 1,1,Color(0,0,0,255))
