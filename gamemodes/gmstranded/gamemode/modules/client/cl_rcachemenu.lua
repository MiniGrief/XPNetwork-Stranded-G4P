...
function PANEL:DrawFrame()
	...
	local CacheSearch = vgui.Create( "DTextEntry" )
	CacheSearch:SetParent( self )
	CacheSearch:SetPos( 190, 580 )
	CacheSearch:SetSize( 100, 15 )
	CacheSearch:SetPlaceholderText( "Search" )
	CacheSearch:SetEditable( true )
	CacheSearch.OnEnter = function( self )
		if not IsValid( CacheInventory ) then return end
		if haveisearchedresource then
			for k, v in SortedPairs( SGS.rcache ) do
				if v <= 0 then continue end
				CacheInventory:AddLine( k, v )
			end
			haveisearchedresource = false
		end
		CacheInventory:Clear()
		for k, v in SortedPairs( SGS.rcache ) do
			if v <= 0 then continue end
			if string.find( k, string.lower(self:GetValue()) ) then
				CacheInventory:AddLine( k, v )
			end
			haveisearchedresource = true
		end
	end
	//CacheSearch:SetUpdateOnType( true )
...
vgui.Register("sgs_rcachemenu", PANEL, "EditablePanel")
