include("shared.lua")
CreateClientConVar( "sgs_radio_volume", "1", true, false )
...
function ENT:Think()
	...
	if self:GetMaxVolume() != radvol then
		radvol = self:GetMaxVolume()
	end
	return true
...
function ENT:GetMaxVolume()
	return GetConVarNumber( "sgs_radio_volume" )
end
...
net.Receive( "radio_play", function( len )
	...
	sound.PlayURL( stationurl, "3d", function( station )
		if IsValid(station) then
			...
			stationent.station:SetVolume( radvol )
