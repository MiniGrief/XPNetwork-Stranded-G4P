...
function ENT:ReloadPlayer()
	...
	self:Reload()
	self.m_pnlWebPage:OpenURL( "https://xpnetwork.online/stranded/player.php" )
end
...
function ENT:PlayVideo( strVideoID, intTime )
	...
	self:LoadVideo()
	if ValidPanel( self.m_pnlWebPage ) then 
		self.m_pnlWebPage:RunJavascript( 'jQuery("#youtube-video-player").tubeplayer("volume",0);' )
	end
end
...
function ENT:ShowMenu()
	...
	self.m_pnlMenu:SetSize( 640, 550 )
...
function Panel:Init()
	...
	self.m_pnlWarnTxt = vgui.Create( "DLabel", self )
	self.m_pnlWarnTxt:SetPos( 25, 500 )
	self.m_pnlWarnTxt:SetText( "If you get an error from YouTube, you are likely not using the chromium branch of GMod!" )
	self.m_pnlWarnTxt:SetFont( "HudHintTextLarge" )
	self.m_pnlWarnTxt:SizeToContents()
...
function Panel:PerformLayout( intW, intH )
	...
	self.m_pnlStopBtn:SetPos( 100, intH -30 )

	self.m_pnlPresetList:SetSize( intW -10, intH -60 -self.m_pnlStopBtn:GetTall() )
